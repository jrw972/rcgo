...
Parameters and variables with types that contain pointers and have foreign dereference mutability are not available in the body of an activation statement because they may represent the state of another component which must be assumed to be invalid in the mutable phase of a transaction.
...
Thus, the result of the move does not have foreign dereference mutability and is available in the body of the activation statement.
...
It then makes the new heap the current heap via the \verb+change+ statement which also makes the root of the heap available in the variable \verb+y+.
...
The \verb+System+ merges the message which moves the root object of a heap to the current heap and returns a pointer to the root object of the heap.
Recall that all components have a default heap which is the current heap upon entering an action, reaction, initializer, or getter.
