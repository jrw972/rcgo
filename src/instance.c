...
{
  // For each instance.
  VECTOR_FOREACH (instance_pos, instance_limit, table->instances, instance_t*)
    {
      ...
      for (binding_pos = type_component_bindings_begin (type), binding_limit = type_component_bindings_end (type);
           ...
           binding_pos = type_component_bindings_next (binding_pos))
        {
          ...
          while (result_pos != result_limit)
            {
              output_instance = lookup_subinstance (table, output_instance, output_port);
          ...
          while (result_pos != result_limit)
            {
              input_instance = lookup_subinstance (table, input_instance, input_reaction);
              input_reaction = *result_pos;
              result_pos = binding_input_next (result_pos);
            }

          add_binding (table, output_instance, output_port, input_instance, binding_get_reaction (binding));
        }
    }
}
  ...
  VECTOR_FOREACH (instance_pos, instance_limit, table->instances, instance_t*)
    {
      ...
      action_t** action_pos;
      action_t** action_end;
      for (action_pos = type_component_actions_begin (type), action_end = type_component_actions_end (type);
