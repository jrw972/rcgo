...
Signature:
  '(' ')'
{ $$ = new ast_signature_type_spec_t (yyloc); }
| '(' ParameterList optional_semicolon ')'
{ $$ = $2; }

ParameterList:
  Parameter
{ $$ = (new ast_signature_type_spec_t (@1))->Append ($1); }
...
{ $$ = new ast_bind_push_port_statement_t (@1, $1, $3); }
| Expression RIGHT_ARROW Expression DOTDOTDOT Expression';'
{ $$ = new ast_bind_push_port_param_statement_t (@1, $1, $3, $5); }
| Expression LEFT_ARROW Expression ';'
{ $$ = new ast_bind_pull_port_statement_t (@1, $1, $3); }

Block: '{' StatementList '}' { $$ = $2; }

StatementList:
  /* empty */ { $$ = new ast_list_statement_t (yyloc); }
...
{ $$ = new ast_activate_statement_t (@1, $2, $3); }

ChangeStatement:
  CHANGE '(' Expression ',' IDENTIFIER ')' Block
{ $$ = new ast_change_statement_t (@1, $3, $5, $7); }
...
ReturnStatement:
  RETURN Expression ';'
{ $$ = new ast_return_statement_t (@1, $2); }

IncrementStatement:
  Expression INCREMENT ';'
{ $$ = new ast_increment_statement_t (@1, $1); }
| Expression DECREMENT ';'
{ $$ = new ast_decrement_statement_t (@1, $1); }

OptionalPushPortCallList:
  /* Empty. */
{ $$ = new ast_list_expr_t (yyloc); }
| PushPortCallList
{ $$ = $1; }

PushPortCallList:
  PushPortCall
{ $$ = (new ast_list_expr_t (@1))->Append ($1); }
| PushPortCallList ',' PushPortCall
{ $$ = $1->Append ($3); }

PushPortCall:
  IDENTIFIER IndexExpression '(' OptionalExpressionList ')'
{ $$ = new ast_indexed_port_call_expr_t (@1, $1, $2, $4); }
| IDENTIFIER '(' OptionalExpressionList ')'
{ $$ = new ast_push_port_call_expr_t (@1, $1, $3); }

IndexExpression: '[' Expression ']' { $$ = $2; }

OptionalExpressionList:
  /* Empty. */   { $$ = new ast_list_expr_t (yyloc); }
| ExpressionList { $$ = $1; }
...
{ $$ = (new ast_list_expr_t (@1))->Append ($1); }
| TypeLitExpression
{ $$ = (new ast_list_expr_t (@1))->Append (new TypeExpression (@1, $1)); }
...
{ $$ = new ast_var_statement_t (@1, $2, $3, $4, $5, $7); }
| VAR IdentifierList Mutability DereferenceMutability Type ';'
{ $$ = new ast_var_statement_t (@1, $2, $3, $4, $5, new ast_list_expr_t (@1)); }
| VAR IdentifierList Mutability DereferenceMutability '=' ExpressionList ';'
{ $$ = new ast_var_statement_t (@1, $2, $3, $4, new ast_empty_type_spec_t (@1), $6); }
| IdentifierList Mutability DereferenceMutability SHORT_ASSIGN ExpressionList ';'
{ $$ = new ast_var_statement_t (@1, $1, $2, $3, new ast_empty_type_spec_t (@1), $5); }

AssignmentStatement:
  Expression '=' Expression ';'
{ $$ = new ast_assign_statement_t (@1, $1, $3); }
| Expression ADD_ASSIGN Expression ';'
{ $$ = new ast_add_assign_statement_t (@1, $1, $3); } /* CHECK */

IfStatement:
  IF Expression Block
  { $$ = new ast_if_statement_t (@1, new ast_empty_statement_t (@1), $2, $3, new ast_list_statement_t (@1)); }
| IF Expression Block ELSE IfStatement
{ $$ = new ast_if_statement_t (@1, new ast_empty_statement_t (@1), $2, $3, $5); }
| IF Expression Block ELSE Block
{ $$ = new ast_if_statement_t (@1, new ast_empty_statement_t (@1), $2, $3, $5); }
| IF SimpleStatement ';' Expression Block
{ $$ = new ast_if_statement_t (@1, $2, $4, $5, new ast_list_statement_t (@1)); }
| IF SimpleStatement ';' Expression Block ELSE IfStatement
{ $$ = new ast_if_statement_t (@1, $2, $4, $5, $7); }
| IF SimpleStatement ';' Expression Block ELSE Block
{ $$ = new ast_if_statement_t (@1, $2, $4, $5, $7); }

WhileStatement:
  FOR Expression Block
{ $$ = new ast_while_statement_t (@1, $2, $3); }

IdentifierList:
  IDENTIFIER
{ $$ = (new ast_identifier_list_t (@1))->Append ($1); }
...
SliceType: '[' ']' ElementType { $$ = new ast_slice_type_spec_t (@1, $3); }

MapType: MAP '[' KeyType ']' ElementType { $$ = new ast_map_type_spec_t (@1, $3, $5); }

KeyType: Type { $$ = $1; }

ComponentType:
  COMPONENT '{' FieldList '}'
{ $$ = $3; static_cast<ast_field_list_type_spec_t*> ($3)->IsComponent = true; }

PushPortType: PUSH Signature { $$ = new ast_push_port_type_spec_t (@1, $2); }

PullPortType:
  PULL Signature DereferenceMutability Type
{ $$ = new ast_pull_port_type_spec_t (@1, $2, $3, $4); }

HeapType: HEAP Type { $$ = new ast_heap_type_spec_t (@1, $2); }

PointerType: '*' Type { $$ = new ast_pointer_type_spec_t (@1, $2); }

TypeLit:
  PointerType       { $$ = $1; }
| TypeLitExpression { $$ = $1; }

TypeName: IDENTIFIER { $$ = new ast_identifier_type_spec_t (@1, $1); }
...
FieldList:
  /* empty */
{ $$ = new ast_field_list_type_spec_t (yyloc); }
| FieldList IdentifierList Type ';'
{ $$ = $1->Append (new ast_identifier_list_type_spec_t (@1, $2, Mutable, Mutable, $3)); }
...
{ $$ = new ast_binary_arithmetic_expr_t (@1, LogicOr, $1, $3); }

AndExpression:
  CompareExpression
{ $$ = $1; }
| CompareExpression LOGIC_AND AndExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, LogicAnd, $1, $3); }

CompareExpression:
  AddExpression
{ $$ = $1; }
| AddExpression EQUAL CompareExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, Equal, $1, $3); }
| AddExpression NOT_EQUAL CompareExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, NotEqual, $1, $3); }
| AddExpression '<' CompareExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, LessThan, $1, $3); }
| AddExpression LESS_EQUAL CompareExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, LessEqual, $1, $3); }
| AddExpression '>' CompareExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, MoreThan, $1, $3); }
| AddExpression MORE_EQUAL CompareExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, MoreEqual, $1, $3); }

AddExpression:
  MultiplyExpression
{ $$ = $1; }
| MultiplyExpression '+' AddExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, Add, $1, $3); }
| MultiplyExpression '-' AddExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, Subtract, $1, $3); }
| MultiplyExpression '|' AddExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, BitOr, $1, $3); }
| MultiplyExpression '^' AddExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, BitXor, $1, $3); }

MultiplyExpression:
  UnaryExpression
{ $$ = $1; }
| UnaryExpression '*' MultiplyExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, Multiply, $1, $3); }
| UnaryExpression '/' MultiplyExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, Divide, $1, $3); }
| UnaryExpression '%' MultiplyExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, Modulus, $1, $3); }
| UnaryExpression LEFT_SHIFT MultiplyExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, LeftShift, $1, $3); }
| UnaryExpression RIGHT_SHIFT MultiplyExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, RightShift, $1, $3); }
| UnaryExpression '&' MultiplyExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, BitAnd, $1, $3); }
| UnaryExpression AND_NOT MultiplyExpression
{ $$ = new ast_binary_arithmetic_expr_t (@1, BitAndNot, $1, $3); }

UnaryExpression:
  PrimaryExpression   { $$ = $1; }
| '+' UnaryExpression { $$ = new ast_unary_arithmetic_expr_t (@1, Posate, $2); }
| '-' UnaryExpression { $$ = new ast_unary_arithmetic_expr_t (@1, Negate, $2); }
| '!' UnaryExpression { $$ = new ast_unary_arithmetic_expr_t (@1, LogicNot, $2); }
| '^' UnaryExpression { $$ = new ast_unary_arithmetic_expr_t (@1, Complement, $2); }
...
{ $$ = new ast_composite_literal_t (@1, $1, $2); }
| IDENTIFIER LiteralValue
{ $$ = new ast_composite_literal_t (@1, new ast_identifier_type_spec_t (@1, $1), $2); }
| '[' DOTDOTDOT ']' ElementType LiteralValue
{ $$ = new ast_composite_literal_t (@1, new ast_array_type_spec_t (@1, new ast_auto_expr_t (@1), $4), $5); }
/* | FunctionLit */
| IDENTIFIER
{ $$ = new ast_identifier_expr_t (@1, $1); }
| '(' Expression ')'
{ $$ = $2; }
...
ElementList:
  Element                 { $$ = (new ast_element_list_t (@1))->Append ($1); }
| ElementList ',' Element { $$ = $1->Append ($3); }

Element:
  Key ':' Value { $$ = new ast_element_t (@1, $1, $3); }
| Value         { $$ = new ast_element_t (@1, new ast_auto_expr_t (@1), $1); }
