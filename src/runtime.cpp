...
void bind (port_t** output_port, void* input_instance, const reaction_t* reaction, int64_t parameter)
{
...
evaluate_statement (executor_t& exec,
                    ast_t* node);
  ...
  struct visitor : public ast_const_visitor_t
  {
    ...
    void visit (const ast_index_expr_t& node)
    {
      struct visitor : public const_type_visitor_t
      {
        ...
        visitor (executor_t& e, const ast_index_expr_t& n) : exec (e), node (n) { }
    ...
    void port_call (const ast_t& node, ast_t* args, const field_t* field, size_t offset = 0)
    {
      ...
      instance_record_t* instance = exec.current_instance ();
      ...
      while (port != NULL)
        {
          ...
          stack_frame_push_pointer (exec.stack (), NULL);
          execute (exec, port->reaction, *(instance_record_t**)port->instance);
        ...
        case ast_binary_arithmetic_expr_t::ADD:
          {
            ...
            struct visitor : public const_type_visitor_t
            {
              ...
              visitor (executor_t& e, const ast_binary_arithmetic_expr_t& n) : exec (e), node (n) { }
                    ...
                    ast_t* node)
{
  struct visitor : public ast_const_visitor_t
  {
    ...
    visitor (executor_t& e) : retval (CONTINUE), exec (e) { }
    ...
    void visit (const ast_add_assign_statement_t& node)
    {
      ...
      struct visitor : public const_type_visitor_t
      {
        executor_t& exec;
        void* ptr;
        visitor (executor_t& e, void* p) : exec (e), ptr (p) { }
         ...
         size_t iota)
{
  ...
  exec.current_instance (record);

  // Push this.
  stack_frame_push_pointer (exec.stack (), exec.get_ptr ());
  // Push iota.
  ...
  while (stack_frame_base_pointer (exec.stack ()) != NULL)
    {
      ...
      stack_frame_push (exec.stack (), symbol_get_offset (this_), symbol_parameter_type (this_)->size ());
      exec.current_instance (*(instance_record_t**)stack_frame_pop_pointer (exec.stack ()));
      ...
      if (trigger->action == TRIGGER_WRITE)
        {
          exec.push (exec.current_instance ());
        }
