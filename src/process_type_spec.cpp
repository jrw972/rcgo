...
long
process_array_dimension (ast::Node* node, ErrorReporter& er, decl::Scope* scope)
{
  process_constant_expression (node, er, scope);
  // Convert to an int.
...
process_parameter_list (Node* node, ErrorReporter& er, decl::Scope* scope, bool is_return)
{
  ...
  struct Visitor : public ast::DefaultNodeVisitor
  {
    ...
    void visit (FieldList& node)
    {
      ...
      if (node.is_component)
        {
          field_list = new Component (scope->package ());
        }
      else
        {
           ...
           ++pos)
        {
          ...
          for (List::ConstIterator pos2 = identifier_list->begin (),
               ...
               ++pos2)
            {
              ...
              if (field == NULL)
                {
                  field_list->append_field (scope->package (), false, identifier, type, TagSet ());
                }
              else
                {
                  error_at_line (-1, 0, id->location.file.c_str (), id->location.line,
    ...
    {
      const decl::ParameterList* parameter_list = process_parameter_list (node.parameters, er, scope, false);
    ...
    {
      const decl::ParameterList* parameter_list = process_parameter_list (node.parameters, er, scope, false);
      const decl::ParameterList* return_parameter_list = process_parameter_list (node.return_parameters, er, scope, true);
      CheckForForeignSafe (er, parameter_list, return_parameter_list);
