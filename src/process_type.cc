...
Error arrayLengthRequired(const Location& a_location) {
  Error error(a_location);
  error.message << "error: an array length is required" << std::endl;
  return error;
}

Error arrayLengthIsNegative(const Location& a_location) {
  Error error(a_location);
  error.message << "error: array length is negative" << std::endl;
  return error;
}

Error doesNotReferToAType(const Location& a_location,
                          const std::string& a_identifier) {
  Error error(a_location);
  error.message << "error: " << a_identifier << " does not refer to a type" << std::endl;
  return error;
}

Error doesNotReferToAnInterface(const Location& a_location,
                                const std::string& a_identifier) {
  Error error(a_location);
  error.message << "error: " << a_identifier << " does not refer to an interface" << std::endl;
  return error;
}

Error keyTypeNotComparable(const Location& a_location) {
  Error error(a_location);
  error.message << "error: key type not comparable" << std::endl;
  return error;
}
...
struct Visitor : public ast::DefaultNodeVisitor {
  ...
  Visitor(type::Factory* a_type_factory, Block* a_block,
      ...
      : type_factory(a_type_factory), block(a_block),
        error_list(a_error_list), struct_type(NULL),
  ...
  void Visit(ast::Array* ast) override {
    const type::Type* element_type =
        ProcessType(ast->element_type, block, type_factory, error_list);
    if (ast->optional_length == NULL) {
      error_list->push_back(arrayLengthRequired(ast->location));
      type = &type::Error::instance;
      return;
    }
    CheckTypes(ast->optional_length, block, type_factory, error_list);
    value::Value * length = ast->optional_length->converted_value();
    if (length->IsError()) {
      type = &type::Error::instance;
      return;
    }
    *length = value::Value::ConvertConstant(*length, &type::Int::instance);
    if (length->IsError()) {
  ...
  void Visit(ast::Pointer* ast) override {
    const type::Type* base_type =
        ProcessType(ast->base_type, block, type_factory, error_list);
  ...
  void Visit(ast::Field* ast) override {
    ...
    for (ast::Node* node : ast->identifier_list) {
      ...
      if (previous_field != NULL) {
        error_list->push_back(DuplicateSymbol(field, previous_field));
      }
    }
  }
  ...
  void Visit(ast::EmbeddedField* ast) override {
    // TODO(jrw972):  Handle recursive types.
    symbol::Symbol* s =
        ProcessQualifiedIdentifier(ast->type_name, *block, error_list);
    if (s == NULL) {
      return;
    }
  ...
  void Visit(ast::Identifier* ast) override {
    symbol::Symbol* s = ProcessQualifiedIdentifier(ast, *block, error_list);
    if (s == NULL) {
      type = &type::Error::instance;
      return;
    }
    ...
    if (DefineSymbol(t, block, type_factory, error_list)) {
      type = t->type();
    } else {
      type = &type::Error::instance;
    }
  ...
  void Visit(ast::Selector* ast) override {
    ...
    symbol::Type* t = symbol::Cast<symbol::Type>(s);
    if (t == NULL) {
      error_list->push_back(doesNotReferToAType(ast->location, s->identifier));
      type = &type::Error::instance;
      return;
    }
    if (DefineSymbol(t, block, type_factory, error_list)) {
  ...
  void Visit(ast::MethodSpec* ast) override {
    ...
    if (previous_method != NULL) {
      error_list->push_back(DuplicateSymbol(method, previous_method));
    }
  }
  ...
  void Visit(ast::AnonymousMethodSpec* ast) override {
    ...
    if (!DefineSymbol(ts, block, type_factory, error_list)) {
      return;
    }
    ...
    for (type::Interface::const_method_iterator pos = it->MethodBegin(),
      ...
      if (previous_method != NULL) {
        error_list->push_back(DuplicateSymbol(new_method, previous_method));
      }
    }
  }
  ...
  void Visit(ast::Map* ast) override {
    ...
    const type::Type* value_type =
        ProcessType(ast->value_type, block, type_factory, error_list);
    if (!IsComparable(key_type)) {
      error_list->push_back(keyTypeNotComparable(ast->location));
    }
...
const type::Type* ProcessType(
    ...
    ErrorList* error_list) {
  Visitor visitor(type_factory, block, error_list);
  ast->Accept(&visitor);
  return visitor.type;
}
...
const type::Function* ProcessFunction(
    ...
    ErrorList* error_list) {
  Visitor tv(type_type_factory, block, error_list);
