...
TEST_CASE ("CouldNotFindPackage::CouldNotFindPackage ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: Could not find package for path \"path\"\n");
}

TEST_CASE ("PackageMismatch::PackageMismatch ()") {
  PackageMismatch err (location, "expected", "actual");
  REQUIRE (std::string (err) == "undefined:1: error: expected package expected but found actual\n");
}
...
TEST_CASE ("RecursiveImport::RecursiveImport ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: recursive import of \"path\" detected.  Import chain to follow:\nundefined:1 imports \"path\" *\n");
}
...
TEST_CASE ("DuplicateSymbol::DuplicateSymbol ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: identifier was previously declared at undefined:1\n");
}

TEST_CASE ("NotDeclared::NotDeclared ()") {
  NotDeclared err (location, "identifier");
  REQUIRE (std::string (err) == "undefined:1: error: identifier was not declared\n");
}

TEST_CASE ("DoesNotReferToAPackage::DoesNotReferToAPackage ()") {
  DoesNotReferToAPackage err (location, "identifier");
  REQUIRE (std::string (err) == "undefined:1: error: identifier does not refer to a package\n");
}

TEST_CASE ("ArrayLengthRequired::ArrayLengthRequired ()") {
  ArrayLengthRequired err (location);
  REQUIRE (std::string (err) == "undefined:1: error: an array length is required\n");
}

TEST_CASE ("ArrayLengthNotAnInt::ArrayLengthNotAnInt ()") {
  ArrayLengthNotAnInt err (location);
  REQUIRE (std::string (err) == "undefined:1: error: array length is not an int\n");
}
...
TEST_CASE ("DoesNotReferToAType::DoesNotReferToAType ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: identifier does not refer to a type\n");
}

TEST_CASE ("DoesNotReferToAnInterface::DoesNotReferToAnInterface ()") {
  DoesNotReferToAnInterface err (location, "identifier");
  REQUIRE (std::string (err) == "undefined:1: error: identifier does not refer to an interface\n");
}
...
TEST_CASE ("SymbolIsDefinedRecursively::SymbolIsDefinedRecursively ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: identifier is defined recursively\n");
}

TEST_CASE ("DoesNotReferToAConstant::DoesNotReferToAConstant ()") {
  DoesNotReferToAConstant err (location, "identifier");
  REQUIRE (std::string (err) == "undefined:1: error: identifier does not refer to a constant\n");
}

TEST_CASE ("IsNotExported::IsNotExported ()") {
  IsNotExported err (location, "identifier");
  REQUIRE (std::string (err) == "undefined:1: error: identifier is not exported\n");
}
...
TEST_CASE ("ExpectedAFieldDeclaration::ExpectedAFieldDeclaration ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: expected a field declaration\n");
}

TEST_CASE ("Expected1::Expected1 ()") {
  SECTION ("a") {
    Expected1 err (location, Token::For);
    REQUIRE (std::string (err) == "undefined:1: error: expected a for\n");
  }
  SECTION ("an") {
    Expected1 err (location, Token::If);
...
TEST_CASE ("IllegalVariadicParameter::IllegalVariadicParameter ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: illegal variadic parameter\n");
}

TEST_CASE ("ExpectedAMethodSpecification::ExpectedAMethodSpecification ()") {
  ExpectedAMethodSpecification err (location);
  REQUIRE (std::string (err) == "undefined:1: error: expected a method specification\n");
}
...
TEST_CASE ("ExpectedASpecificationOrGroup::ExpectedASpecificationOrGroup ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: expected a specification or group\n");
}
...
TEST_CASE ("UnexpectedExpressionList::UnexpectedExpressionList ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: unexpected expression list\n");
}

TEST_CASE ("Expected2::Expected2 ()") {
  Expected2 err (location, Token::For, Token::If);
...
TEST_CASE ("ExtraCharactersInRuneLiteral::ExtraCharactersInRuneLiteral ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: extra characters in rune literal\n");
}
...
TEST_CASE ("TooFewOctalDigitsInRuneLiteral::TooFewOctalDigitsInRuneLiteral ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: too few octal digits in rune literal\n");
}

TEST_CASE ("TooFewHexadecimalDigitsInRuneLiteral::TooFewHexadecimalDigitsInRuneLiteral ()") {
  TooFewHexadecimalDigitsInRuneLiteral err (location);
  REQUIRE (std::string (err) == "undefined:1: error: too few hexadecimal digits in rune literal\n");
}

TEST_CASE ("InvalidUnicodeCodePoint::InvalidUnicodeCodePoint ()") {
  InvalidUnicodeCodePoint err (location);
  REQUIRE (std::string (err) == "undefined:1: error: invalid unicode code point\n");
}
...
TEST_CASE ("IncompleteStringLiteral::IncompleteStringLiteral ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: incomplete string literal\n");
}

TEST_CASE ("ExtraCharactersInToken::ExtraCharactersInToken ()") {
  ExtraCharactersInToken err (location);
  REQUIRE (std::string (err) == "undefined:1: error: extra characters in token\n");
}
...
TEST_CASE ("UnterminatedGeneralComment::UnterminatedGeneralComment ()") {
  ...
  REQUIRE (std::string (err) == "undefined:1: error: unterminated comment\n");
}
