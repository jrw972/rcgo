...
symbol_t* symbol_make_undefined (string_t identifier, SymbolKind kind, ast_t* defining_node);
...
symbol_t *symbol_make_type (string_t identifier, type_t * type, ast_t* defining_node);

type_t *symbol_get_type_type (const symbol_t * symbol);

symbol_t *symbol_make_typed_constant (string_t identifier, typed_value_t value, ast_t* defining_node);

typed_value_t symbol_typed_constant_value (const symbol_t * symbol);

symbol_t *symbol_make_untyped_constant (string_t identifier, untyped_value_t value, ast_t* defining_node);

untyped_value_t symbol_untyped_constant_value (const symbol_t * symbol);

symbol_t *symbol_make_instance (string_t identifier, const type_t * type, ast_t* defining_node);
...
typedef enum {
  ParameterOrdinary,
  ParameterThis,
  ParameterThisDuplicate,
} ParameterKind;

symbol_t *symbol_make_parameter (string_t identifier, type_t* type, ast_t* defining_node, ParameterKind kind);
