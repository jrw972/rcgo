...
static ast_t *
make_type_spec (AstTypeSpecificationKind kind,
                size_t children_count)
{
...
static ast_t *
make_expr (AstExpressionKind kind,
           size_t children_count)
{
...
static ast_t *
make_stmt (AstStatementKind kind,
           size_t children_count)
{
...
static ast_t *
make_bind_stmt (AstBindStatementKind kind,
                size_t children_count)
{
...
size_t ast_children_count (const ast_t* node)
{
...
const char* ast_file (const ast_t* node)
{
  return node->file;
}

unsigned int ast_line (const ast_t* node)
{
  return node->line;
}

void ast_set_symtab (ast_t* node,
                      symtab_t* symtab)
{
...
symtab_t* ast_get_symtab (const ast_t* node)
{
  return node->symtab;
}

void ast_set_symbol (ast_t* node, symbol_t* symbol)
{
  node->symbol = symbol;
}

symbol_t* ast_get_symbol (ast_t* node)
{
  return node->symbol;
}
...
ast_t *
ast_make_trigger_stmt (ast_t* expr_list,
                        ast_t * stmt)
{
...
{
  assert (node->kind == AstExpression);
  assert (node->expression.kind != AstTypedLiteral && node->expression.kind != AstUntypedLiteral);
